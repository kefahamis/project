...
class Kirki_Control_Repeater extends WP_Customize_Control {
	...
	protected function l10n( $config_id = false ) {
		$translation_strings = array(
			'row'               => esc_attr__( 'row', 'i-max' ),
			'add-new'           => esc_attr__( 'Add new', 'i-max' ),
			'select-page'       => esc_attr__( 'Select a Page', 'i-max' ),
			/* translators: %s represents the number of rows we're limiting the repeater to allow. */
			'limit-rows'        => esc_attr__( 'Limit: %s rows', 'i-max' ),
			'hex-value'         => esc_attr__( 'Hex Value', 'i-max' ),
			'no-image-selected' => esc_attr__( 'No Image Selected', 'i-max' ),
			'remove'            => esc_attr__( 'Remove', 'i-max' ),
			'add-image'         => esc_attr__( 'Add Image', 'i-max' ),
			'change-image'      => esc_attr__( 'Change Image', 'i-max' ),
			'no-file-selected'  => esc_attr__( 'No File Selected', 'i-max' ),
			'add-file'          => esc_attr__( 'Add File', 'i-max' ),
			'change-file'       => esc_attr__( 'Change File', 'i-max' ),
		);
